...
class SimpleTcpClient(SDTcpClient):
    ...
    def update(self,action):
        ...
        if action.find("FWD") != -1: # Forward
            if action == "FWD":
                forward = 0.40
            elif action == "FWD REDUCED":
                forward = 0.10
            else:
                forward = 0.30
        elif action == "BACK": # Backward
            forward = -0.30
        elif action.find("LEFT") != -1 and action.find("TURN") == -1: # Left
            lateral = -0.30
        elif action.find("RIGHT") != -1 and action.find("TURN") == -1: # Right
            lateral = 0.30
        elif action.find("UP") != -1 and action.find("PITCH") == -1: # Up
            up = 0.30
        elif action.find("DOWN") != -1 and action.find("PITCH") == -1: # Down
            up = -0.30
        elif action.find("TURN LEFT") != -1: # Turn left (Yaw)
            turn = -0.30
        elif action.find("TURN RIGHT") != -1: # Turn right (Yaw)
            turn = 0.30
        elif action == "PITCH UP": # Pitch Up
            pitch = -0.30
        elif action == "PITCH DOWN": # Pitch Down
            pitch = 0.30
        elif action == "ROLL CW": # Roll Clockwise
            roll = -0.10
        elif action == "ROLL COUNTER_CW": # Roll Counter Clockwise
            roll = 0.10

        self.send_controls(up_force=up,lateral_force=lateral,forward_force=forward,roll_force=roll,pitch_force=pitch,yaw_force=turn)
