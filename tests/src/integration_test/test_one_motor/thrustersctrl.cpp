...
ThrustersControl::ThrustersControl(){
    ...
    if(wiringPiSetup() == -1) cout << "Não conectou" << endl;
...
void ThrustersControl::initializeThrusters(){
    for(int i = 0; i < PINS.size(); i++){
        if(i == 2 || i == 5) thrusters.emplace_back(PINS[i], 0);
...
void ThrustersControl::defineAction(Decision decision){
    cout << "Action: " + actionToString(decision.action) + ", Power: " + to_string(decision.value) << endl;
    stabilizeHori = true;
    stabilizeVert = true;
