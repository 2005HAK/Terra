    ...
    try {
	    ...
	    // tem que tratar o caso de 
            try {
                ...
                logMessage(std::string("Erro ao interpretar JSON: ") + e.what());
            }
        }
    } catch (exception& e) {
        logMessage(std::string("Erro: ") + e.what());
    }
}
...
void YoloCtrl::switchCam(int chooseCam){
    ...
    if(cam == chooseCam){
        logMessage(std::string("Value changed to: ") + (cam == 0 ? "Front camera" : "Bottom camera"));
    } else{
        // gerar erro
    }
}
...
vector<Object> YoloCtrl::process_json(const json& received_json){
    ...
    try {
        ...
        logMessage(std::string("Erro ao processar JSON: ") + e.what());
    }
    return results;
}
